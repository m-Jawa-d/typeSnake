/*
  WebGL fluid simulation — adapted from PavelDoGreat/WebGL-Fluid-Simulation (MIT, © 2017 Pavel Dobryakov).

  Differences from the original script:
  - packaged as a factory that returns a destroy() handle instead of running on import
  - pointer input is tracked on window (hover, no click needed) so the canvas can stay pointer-events:none
  - transparent output composited over the page instead of an opaque background/checkerboard
  - no dat.GUI, screenshot capture, analytics or keyboard shortcuts
*/
...
export default function createFluidSimulation(canvas, options = {}) {
  ...
  function getWebGLContext(target) {
    ...
    if (!isWebGL2) context = target.getContext('webgl', params) || target.getContext('experimental-webgl', params);
  ...
  // Everything allocated on the GPU is tracked so destroy() can free it without
  // killing the context — the same canvas may be re-initialised later.
  const resources = { shaders: [], programs: [], textures: [], framebuffers: [], buffers: [] };
  ...
  resources.buffers.push(quadVertexBuffer, quadIndexBuffer);
  ...
  const ditheringTexture = createFlatTexture();
  ...
  function createFlatTexture() {
    ...
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, 1, 1, 0, gl.RGB, gl.UNSIGNED_BYTE, new Uint8Array([127, 127, 127]));
    resources.textures.push(texture);
  ...
  function render() {
    ...
    // The page shows through the canvas, so the frame starts fully transparent
    // instead of being covered by an opaque background pass.
    gl.bindFramebuffer(gl.FRAMEBUFFER, null);
    gl.viewport(0, 0, gl.drawingBufferWidth, gl.drawingBufferHeight);
    gl.clearColor(0.0, 0.0, 0.0, 0.0);
    gl.clear(gl.COLOR_BUFFER_BIT);
  ...
  function resetPointer(pointer, posX, posY) {
    pointer.texcoordX = posX / canvas.width;
    pointer.texcoordY = 1.0 - posY / canvas.height;
    pointer.prevTexcoordX = pointer.texcoordX;
    pointer.prevTexcoordY = pointer.texcoordY;
  ...
  // The first move after the pointer (re)enters the window would otherwise splat a
  // streak across everything it skipped.
  let pointerNeedsReset = true;

  const onMouseMove = (event) => {
    const { x, y } = pointerPosition(event.clientX, event.clientY);
    if (pointerNeedsReset) {
      pointerNeedsReset = false;
      resetPointer(pointers[0], x, y);
      return;
    }
  ...
  const onMouseOut = (event) => {
    if (event.relatedTarget == null) pointerNeedsReset = true;
  };

  const onTouchMove = (event) => {
    const touches = event.targetTouches;
    for (let i = 0; i < touches.length; i++) {
      while (pointers.length <= i + 1) pointers.push(createPointer());
      const { x, y } = pointerPosition(touches[i].clientX, touches[i].clientY);
  ...
  return { destroy, splats: (amount) => splatStack.push(amount) };
}
